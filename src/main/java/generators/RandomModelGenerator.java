...
public class RandomModelGenerator {
    ...
    public static <T> T generate(Class<T> clazz) {
        try {
            ...
            for (Field field : getAllFields(clazz)) {
                ...
                if (rule != null) {
                    // Если аннотация ЕСТЬ, проверяем её параметры
                    if (!rule.regex().isEmpty()) {
                        value = generateFromRegex(rule.regex(), field.getType());
                    } else if (rule.min() != Double.MIN_VALUE || rule.max() != Double.MAX_VALUE) {
                        value = generateFromRange(rule.min(), rule.max(), field.getType());
                    } else {
                        value = generateRandomValue(field);
                    }
                } else {
                    // Если аннотации НЕТ (как у поля id), просто генерируем рандом
                    value = generateRandomValue(field);
                }
    ...
    private static Object generateFromRange(double min, double max, Class<?> type) {
        double generated = min + (random.nextDouble() * (max - min));

        if (type.equals(BigDecimal.class)) {
            // Округляем до 2 знаков для банковских операций
            return BigDecimal.valueOf(generated).setScale(2, RoundingMode.HALF_UP);
        } else if (type.equals(Integer.class) || type.equals(int.class)) {
